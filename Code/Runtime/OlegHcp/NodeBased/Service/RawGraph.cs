...
namespace OlegHcp.NodeBased.Service
{
    public abstract class RawGraph : ScriptableObject
    {
        ...
        internal RawNode[] NodeArray => _nodes;
        internal Dictionary<int, RawNode> NodeDict => _dict ?? (_dict = _nodes.ToDictionary(key => key.Id, value => value));
        ...
        {
            if (NodeDict.TryGetValue(id, out RawNode value) && value.IsRegular())
                return value;
